...
const FilterMenu: React.FC<PropsTypes> = ({ setTagId }) => {
  ...
  const fetchTags = async () => {
    ...
    try {
      const response = await makeGetRequest(urls.productsTags);
  ...
  return (
    <Dropdown>
      <MenuButton>Choose Tag</MenuButton>

      <Menu slots={{ listbox: AnimatedListbox }}>
        {loading ? (
          <Loader />
        ) : tags && tags.data && tags?.data?.length > 0 ? (
          tags?.data.map((tag: TagsProps) => (
